...
def main():
    ...
    if pizza_type == 'Greek':
        total_price *= 0.945  # 5.5% off
    elif pizza_type == 'Sicilian':
        total_price *= 0.965  # 3.5% off

    if c_choice == '3':
        total_price *= 0.94  # 6% extra off for provolone cheese

    if s_choice == '1':
        total_price *= 0.96  # 4% extra off for Plum tomato sauce

    print('Price of the requested pizza is $', round(total_price, 2))
    print("Name: Moksh Malik \n Enrollment Number: E22MCAG0020")
